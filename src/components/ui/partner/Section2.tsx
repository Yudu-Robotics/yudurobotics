    ...
    <div className="w-full flex flex-col justify-center items-center">
      <div className="w-full flex lg:flex-row flex-col text-black ">
        <div className="lg:w-1/3 px-4">
          <div className="font-cobaltRidge text-5xl md:text-6xl lg:text-7xl">
            {highlightWords(heading, [heading], "text-[#0A041A]")}
          </div>
          <div className="font-[var(--Fontfamilyfont-family-body)] text-[var(--Fontsizetext-md)] font-normal leading-[var(--Lineheighttext-md)] text-left decoration-skip-ink-none text-gray-500 mt-3">
            {subhading}
          </div>
        </div>
        <div className="grid mt-8 lg:mt-0 sm:grid-cols-1 lg:grid-cols-2 grid-cols-1 lg:w-3/4 lg:px-12">
          {data.map((item, index) => (
            <div key={index} className="p-5">
              ...
              <h2 className="mt-5 mb-2 text-lg font-semibold font-tthoves-semiBold ">
                {item.title}
              </h2>
              <p className="mt-4 text-base font-tthoves  text-gray-500">
                {item.desc}
              </p>
            </div>
          ))}
